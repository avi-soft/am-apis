...
public class PaymentService {
    ...
    public double[] balances(Long spId)
    {
        ...
        Query queryAmountCarryOver = entityManager.createNativeQuery("SELECT SUM(e.carry_over) FROM Earnings e WHERE e.date < :inputDate AND e.provider_id = :id AND settled = true");
        queryAmountCarryOver.setParameter("inputDate", java.sql.Date.valueOf(firstOfMonth));
        queryAmountCarryOver.setParameter("id", spId);
        Double lastMonthCarryOver = (Double) queryAmountCarryOver.getSingleResult();
        ...
        Query queryAmountCarryOver2 = entityManager.createNativeQuery("SELECT SUM(e.carry_over) FROM Earnings e WHERE e.date >=:inputDate AND e.provider_id = :id AND settled = true");
        ...
        Double thisMonthCarryOver = (Double) queryAmountCarryOver2.getSingleResult();
