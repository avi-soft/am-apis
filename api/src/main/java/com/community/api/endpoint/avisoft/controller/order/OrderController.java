    ...
    public ResponseEntity<?>manuallyAssignOrder(@PathVariable Long orderId,@RequestBody ManualAssignmentDetails manualAssignmentDetails) {
        try {
            ...
            for (Field field : manualAssignmentDetails.getClass().getDeclaredFields()) {
                ...
                Object value = field.get(manualAssignmentDetails); // Get the value of the field
            ...
            Role role=null;
            if(manualAssignmentDetails.getAssigneeRole()!=null)
            {
                role =roleService.getRoleByRoleId(manualAssignmentDetails.getAssigneeRole());
                if(role==null)
            ...
            {
                serviceProvider = entityManager.find(ServiceProviderEntity.class,manualAssignmentDetails.getAssignee());
            ...
            {
                customAdmin = entityManager.find(CustomAdmin.class, manualAssignmentDetails.getAssignee());
                if (customAdmin == null)
                    return ResponseService.generateErrorResponse("Admin with the provided id not found", HttpStatus.NOT_FOUND);
            }
            CreateTicketDto createTicketDto=new CreateTicketDto();
            createTicketDto.setAssignee(manualAssignmentDetails.getAssignee());
            createTicketDto.setTicketState(manualAssignmentDetails.getTicketState());
            createTicketDto.setTicketStatus(manualAssignmentDetails.getTicketStatus());
            createTicketDto.setAssigneeRole(role);
            createTicketDto.setTargetCompletionDate(manualAssignmentDetails.getTargetCompletionDate());
